shinyServer(function(input, output, session) {
        ...
        if (isTRUE(input$pricingActualizar_conf)) {
            withProgress(value = 0, message = "Actualizando pricing...", min = 0, max = 1.1, {
                ...
                while (i <= length(pricingList$id)) {
                    drive_download(file = as_id(pricingList$id[i]), path = paste0("datos/", pricingList$name[i]), overwrite = T)
                    i = i + 1
    ...
    observeEvent(input$pricingSelect, {
        pricing$bd = as.data.table(readr::read_csv(paste0("datos/", input$pricingSelect, ".csv"), na = c("-", "#N/A", "#DIV/0!", "NA")))
    ...
    observeEvent(input$dashNT_actualizar_conf, {
        if (isTRUE(input$dashNT_actualizar_conf)) {
            ...
            withProgress(value = 0, message = "Actualizando notas técnicas...", {
                ...
                incProgress(0.3)
                saveRDS(mapaValoresNT(as.data.table(sheets_read(nts_path, sheet = "INDICE", col_types = "ccdcccd"))) %>% layout(autosize = TRUE), "datos/NTmapa.rds")
            })
